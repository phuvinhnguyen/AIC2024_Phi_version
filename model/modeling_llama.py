...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Support both input_ids and inputs_embeds at the same time (concatenate)
        if input_ids is not None and inputs_embeds is not None:
            input_embeds_from_ids = self.embed_tokens(input_ids)
            inputs_embeds = torch.cat([inputs_embeds, input_embeds_from_ids], dim=1)
            input_ids = None  # Already handled
        elif inputs_embeds is None and input_ids is not None:
            inputs_embeds = self.embed_tokens(input_ids)
        elif inputs_embeds is not None and input_ids is None:
            pass  # Only embeddings provided
        else:
